...
request.onload = function () {
    ...
    for (var i = 0; i < 32; i++) {
        ...
        arr32.push(matzip['members'][arrIndex[randomNum]]['name']);
        arrIndex.splice(randomNum, 1);
    }

    //맨 처음 상태 설정
    Btn1.innerText = arr32[count32];
    Btn1Img.src = `./images/${arr32[0]}.jpg`;
    Btn1.appendChild(Btn1Img);

    Btn2.innerText = arr32[count32 + 1];
    Btn2Img.src = `./images/${arr32[1]}.jpg`;
    Btn2.appendChild(Btn2Img);

    viewround();

    function handleClick(event) {
        if (count32 < 32) { //32강전이 진행 중
            arr16.push(event.target.parentElement.innerText);
            count32 += 2;
            console.log("arr16: ", arr16);
        } else if ((count32 == 32) && (count16 < 16)) { //32강이 끝나고 16강 진행 중
            arr8.push(event.target.parentElement.innerText);
            count16 += 2;
            console.log("arr8: ", arr8);
        } else if ((count32 == 32) && (count16 == 16) && (count8 < 8)) { //16강 끝나고 8강 진행 중
            arr4.push(event.target.parentElement.innerText);
            count8 += 2;
            console.log("arr4: ", arr4);
        } else if ((count32 == 32) && (count16 == 16) && (count8 == 8) && (count4 < 4)) { //8강 끝나고 4강 진행 중
            arr2.push(event.target.parentElement.innerText);
            count4 += 2;
            console.log("arr2: ", arr2);
        } else if ((count32 == 32) && (count16 == 16) && (count8 == 8) && (count4 == 4) && (count2 < 2)) { //4강 끝나고 결승 진행 중
    ...
    function drawBtn() {
        if (count32 < 32) { //32강전이 진행 중
            Btn1.innerText = arr32[count32];
            Btn2.innerText = arr32[count32 + 1];
            Btn1Img.src = `./images/${arr32[count32]}.jpg`;
            Btn2Img.src = `./images/${arr32[count32 + 1]}.jpg`;
            Btn1.appendChild(Btn1Img);
            Btn2.appendChild(Btn2Img);
        } else if ((count32 == 32) && (count16 < 16)) { //32강이 끝나고 16강 진행 중
            ...
            Btn1.appendChild(Btn1Img);
            Btn2.appendChild(Btn2Img);
        } else if ((count32 == 32) && (count16 == 16) && (count8 == 8) && (count4 == 4) && (count2 == 2)) {
    ...
    function viewround() {
        if (count32 < 32) {
            roundTitle.innerHTML = `식당 이상형 월드컵! 32강 (${count32 / 2 + 1}/16)`;
        } else if ((count32 == 32) && (count16 < 16)) {
            roundTitle.innerHTML = `식당 이상형 월드컵! 16강 (${count16 / 2 + 1}/8)`;
        } else if ((count32 == 32) && (count16 == 16) && (count8 < 8)) {
            roundTitle.innerHTML = `식당 이상형 월드컵! 8강 (${count8 / 2 + 1}/4)`;
        } else if ((count32 == 32) && (count16 == 16) && (count8 == 8) && (count4 < 4)) {
